...
//QByteArray DataPackage::addLandLordCards(int landlordID, int num, int landLordCards[])
//{
//    QByteArray bytes;
//    QDataStream out(&bytes,QIODevice::WriteOnly);
//    out.setVersion(QDataStream::Qt_5_2);
//    out<<(quint16)0;
//    out<<DataType::addLandLordCards;
//    out<<landlordID;
//    out<<num;
//    for(int i=0;i<num;i++)
//        out<<landLordCards[i];
...
//轮谁叫地主报文装包
//参数表示现在应该轮到叫地主的玩家的编号
QByteArray DataPackage::whoCall(int thID,const vector<int>& landLordCards)
{
    QByteArray bytes;
    QDataStream out(&bytes,QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_5_2);
    out<<(quint16)0;
    out<<DataType::WhoCall;
    out<<thID;
    for(int i=0;i<3;i++)
        out<<landLordCards[i];
    out.device()->seek(0);
...
//叫地主信息报文装包
//参数分别是(出牌的玩家编号,是否叫地主)
//  当客户端发送叫地主请求时,playerID表示该你的游戏编号,isCall表示是否叫地主
//  当客户端收到广播时,playerID则表示刚才出牌成功的玩家的编号
QByteArray DataPackage::jiaodizhu(int playerID, bool isCall,int landLordCards[])
{
    QByteArray bytes;
    QDataStream out(&bytes,QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_5_2);
    out<<(quint16)0;
    out<<DataType::CallLord;
    out<<playerID;
    out<<isCall;
    for(int i=0;i<3;i++)
        out<<landLordCards[i];
...
QByteArray DataPackage::callFeedBack(bool isCall/*,int landLordCards[3]*/)
{
    QByteArray bytes;
    QDataStream out(&bytes,QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_5_2);
    out<<(quint16)0;
    out<<DataType::CallFeedBack;
    out<<isCall;
//    for(int i=0;i<3;i++)
//        out<<landLordCards[i];
    out.device()->seek(0);
