...
simpleMethod::simpleMethod(Player* player,QList<card> cards)
{
    m_player=player;
    m_cards=cards;
}
...
int simpleMethod::CountOfPoint(QList<card> cards,CardPoint point)
{
    int count=0;
...
QList<QList<card> > simpleMethod::FindHand(Hand hand, bool beat)
{
    ...
    else if (handType == Hand_Triple_Single)
    {
        ...
        if (!findCardsArray.isEmpty())
        {
            ...
            simpleMethod st(m_player, remainCards);
...
// 找出count张点数为point的牌
QList<card> simpleMethod::FindSamePointCards(CardPoint point, int count)
{
...
// 找出所有点数相同张数为count的牌组
QList<QList<card> > simpleMethod::FindCardsByCount(int count)
{
...
// 找出所有在范围内的牌
QList<card> simpleMethod::GetRangeCards(CardPoint beginPoint, CardPoint endPoint)
{
...
// 找到能构成顺子的最小起点牌的牌组
QList<card> simpleMethod::GetFirstSeqSingleRange()
{
    int seqCount = 0;
...
void simpleMethod::PickSeqSingles(QList<QList<QList<card>  > >& allSeqRecord, const QList<QList<card> >& seqInherited, QList<card> cards)
{
    QList<QList<card> > allSeqScheme = simpleMethod(m_player, cards).FindHand(Hand(Hand_Seq_Single, Card_Begin, 0), false);
...
QList<QList<card> > simpleMethod::PickOptimalSeqSingles()
{
    QList<QList<QList<card> > > seqRecord;
    QList<QList<card> > seqInherited;
    simpleMethod(m_player, m_cards).PickSeqSingles(seqRecord, seqInherited, m_cards);
    ...
    for (int i = 0; i < seqRecord.size(); i++)
    {
        ...
        QList<QList<card> > singleArray = simpleMethod(m_player, backupCards).FindCardsByCount(1);
...
// 主动出牌
QList<card> simpleMethod::PlayFirst()
{
    Hand hand(m_cards);
    ...
    if (!seqSingleRange.isEmpty())
    {
        ...
        //清除炸弹和三个
        QList<QList<card> > cards4=simpleMethod(m_player,left).FindCardsByCount(4);

        for(int i=0;i<cards4.size();i++)
            for(int j=0;j<cards4[i].size();j++)
                left.removeOne(cards4[i][j]);

        QList<QList<card> > cards3=simpleMethod(m_player,left).FindCardsByCount(3);

        for(int i=0;i<cards3.size();i++)
            for(int j=0;j<cards3[i].size();j++)
                left.removeOne(cards3[i][j]);

        //     left.Remove(simpleMethod(m_player, left).FindCardsByCount(4));
        //     left.Remove(Strategy(m_player, left).FindCardsByCount(3));



        QList<QList<card> > optimalSeq = simpleMethod(m_player, left).PickOptimalSeqSingles();
        if (!optimalSeq.isEmpty())
        {
            int oriSingleCount = simpleMethod(m_player, left).FindCardsByCount(1).size();
            ...
            int leftSingleCount = simpleMethod(m_player, left).FindCardsByCount(1).size();
    ...
    QList<QList<card> > bombArray = simpleMethod(m_player, leftCards).FindHand(Hand(Hand_Bomb, Card_Begin, 0), false);
    ...
    QList<QList<card> > planeArray = simpleMethod(m_player, leftCards).FindHand(Hand(Hand_Plane, Card_Begin, 0), false);
    ...
    QList<QList<card> > tripleArray = simpleMethod(m_player, leftCards).FindHand(Hand(Hand_Triple, Card_Begin, 0), false);
    ...
    QList<QList<card> > seqPairArray = simpleMethod(m_player, leftCards).FindHand(Hand(Hand_Seq_Pair, Card_Begin, 0), false);
    ...
    if (hasPlane)
    {
        ...
        {
            QList<card>  pair = simpleMethod(m_player, leftCards).FindSamePointCards(point, 2);
            if (!pair.isEmpty())
            {
                pairArray << pair;
                if (pairArray.size() == 2)
        ...
        else
        {
            ...
            for (CardPoint point = Card_3; point <= Card_A; point = CardPoint(point + 1))
            {
                ...
                {
                    QList<card>  single = simpleMethod(m_player, leftCards).FindSamePointCards(point, 1);
                    if (!single.isEmpty())
                    {
                        singleArray << single;
...
// 大过指定牌型
QList<card> simpleMethod::PlayBeatHand(Hand hand)
{
    // 先固定住最优顺子，从余下牌中打出
    QList<card>  left = m_cards;
    QList<QList<card> > cardlll=simpleMethod(m_player,left).PickOptimalSeqSingles();
    for(int i=0;i<cardlll.size();i++)
        for(int j=0;j<cardlll[i].size();j++ )
            left.removeOne(cardlll[i][j]);

    //left.Remove(simpleMethod(m_player, left).PickOptimalSeqSingles());

    if (hand.getHandType() == Hand_Single)	// 如果压单牌，尽量从单张牌中挑
    {
        QList<QList<card> > singleArray = simpleMethod(m_player, left).FindCardsByCount(1);
        for (int i = 0; i < singleArray.size(); i++)
        {
    ...
    {
        QList<QList<card> > pairArray = simpleMethod(m_player, left).FindCardsByCount(2);
        for (int i = 0; i < pairArray.size(); i++)
        {
    ...
    Player* nextPlayer = m_player->getNextPlayer();
    QList<QList<card> > beatCardsArray = simpleMethod(m_player, left).FindHand(hand, true);
    ...
    {
        beatCardsArray = simpleMethod(m_player, m_cards).FindHand(hand, true);
        if (!beatCardsArray.isEmpty())
        {
            if (m_player->getRole() != nextPlayer->getRole() &&
                    nextPlayer->getCards().size() <= 2)
...
QList<card> simpleMethod::MakeDecision()
{
