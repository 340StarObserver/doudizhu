...
void GamePanel::Init()
{
    ...
    m_clock->setPixmap(QPixmap(":/img/res/clock.jpg"));
    m_clock->move(width()/2-150,height()-180);
    m_clock->hide();
    ...
    connect(m_start,SIGNAL(clicked(bool)),this,SLOT(on_start_clicked(bool)));
    ...
    connect(m_bet0,SIGNAL(clicked(bool)),this,SLOT(on_bet0_clicked(bool)));
    connect(m_bet1,SIGNAL(clicked(bool)),this,SLOT(on_bet1_clicked(bool)));
    connect(m_bet2,SIGNAL(clicked(bool)),this,SLOT(on_bet2_clicked(bool)));
    connect(m_bet3,SIGNAL(clicked(bool)),this,SLOT(on_bet3_clicked(bool)));
    ...
    m_pass->setText("过");
    m_pass->move(width()/2-100,height()-card_height-40);
    m_playhand->move(width()/2+40,height()-card_height-40);

    connect(m_playhand,SIGNAL(clicked(bool)),this,SLOT(on_playhand_clicked(bool)));
    connect(m_pass,SIGNAL(clicked(bool)),this,SLOT(on_pass_clicked(bool)));
    ...
    connect(m_continue,SIGNAL(clicked(bool)),this,SLOT(on_continue_clicked(bool)));
    ...
    noCallMusic=new QSound(":/sound/res/noCall.wav",this);
    callLordMusic=new QSound(":/sound/res/callLord.wav",this);
    ...
    m_gameprocess->getDownPlayer()->betLabel->move(width()/2-70,(height()-card_height-150));
    ...
    m_gameprocess->getDownPlayer()->passLabel->move(width()/2-70,(height()-card_height-150));
...
void GamePanel::CutCardPic(int x,int y,card c)
{
...
void GamePanel::StartProcess(GameProcess::GameStatus status)
{
    switch(status)
    {
    ...
    case (GameProcess::PlayingHand):
        ...
        for(int i=0;i<3;i++)
        {
            ...
            m_restThreeCards[i]->move(350+i*100,5);
...
void GamePanel::SetPlayerStatus(Player *player, Player::PlayerStatus status)
{
...
void GamePanel::on_start_clicked(bool state)
{
    bkMusic->stop();
    m_start->hide();                            //隐藏开始按钮
...
void GamePanel::PickingCardsAnimation(Player* player)
{
...
void GamePanel::UpdateCards(Player *player)
{
...
void GamePanel::PlayerStartPlayingHand()
{    
    /*从选中的牌图片中得到要打出去的牌组*/
...
void GamePanel::PlayerStartCallingLord()
{
    //QTimer::singleShot(10000,this,SLOT(NoOneCallLord()));
...
void GamePanel::PlayingHand(Player* player,QList<card> cards)
{
...
void GamePanel::CallingLord(Player *player, int bet)
{
    if(bet ==3)
...
void GamePanel::on_bet0_clicked(bool state)
{
    m_gameprocess->getDownPlayer()->callingLord(0);
}

void GamePanel::on_bet1_clicked(bool state)
{
    m_gameprocess->getDownPlayer()->callingLord(1);
}

void GamePanel::on_bet2_clicked(bool state)
{
    m_gameprocess->getDownPlayer()->callingLord(2);
}

void GamePanel::on_bet3_clicked(bool state)
{
    m_gameprocess->getDownPlayer()->callingLord(3);
}

void GamePanel::on_playhand_clicked(bool state)
{
    PlayerStartPlayingHand();
}

void GamePanel::on_pass_clicked(bool state)
{
    //选中的牌降下
    CardPicture* cardpic;
...
void GamePanel::on_continue_clicked(bool state)
{
